...
export class BaseHands {
  ...
  typeTextonDomWithIndex(locatorName: string, locatorValue: string, text: string, index: number) {
    cy.get(`[${locatorName}="${locatorValue}"]`).eq(index).type(text, { force: true });
    return this;
  }

  clickOnAttributeWithValue(attribute: string, value: string) {
    cy.get(`[${attribute}=${value}]`).click();
    return this;
  }

  scrollWithDomElement(dom: string){
    cy.get(dom).scrollIntoView().should('be.visible');
    return this;
  }

  clickOnSelectWithValue(id: string, value: string){
    cy.get(`#${id}`).select(value);
    return this;
  }

  selectInDropdownWithDomAndOption(dom: string, option:string){
    cy.get(dom).select(option,{ force: true });
    return this;
  }

  clickOnCssProperty(css: string){
    cy.get(`${css}`).click();
    return this;
  }
}
